...
public class BrightNodeTileEntity extends TileEntity implements ITickableTileEntity {
    ...
    public void tick() {
        if(world.isRemote()) {
            ...
            if(priority > 1 && isBlockOnCursor(player, playerWorld)) {
                playerWorld.addParticle(ArcanaParticles.ASPECT_LIGHT_PARTICLE, true, getParticleX(), getParticleY() + 1, getParticleZ(), 0, 0, 0);
            }
        }
    }
    ...
    private boolean isBlockOnCursor(ClientPlayerEntity player, World playerWorld) {
        ...
        return playerWorld.getBlockState(new BlockPos(result.getHitVec())).getBlock().getRegistryName().toString().equals("arcana:bright_node");
    }
}
