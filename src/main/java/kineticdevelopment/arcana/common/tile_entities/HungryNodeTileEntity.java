    ...
    public void tick() {
        if(world.isRemote()) {
            ...
            if(priority >= ArcanaHelper.ASPECT_GOGGLES_PRIORITY) {
                ...
                playerWorld.addParticle(ArcanaParticles.HUNGRY_NODE_PARTICLE, true, particleLocation.getX(), particleLocation.getY(), particleLocation.getZ(), 0, 0, 0);
            }
            if(priority >= ArcanaHelper.GOGGLES_OF_REVEALING_PRIORITY && ArcanaHelper.isBlockOnCursor(instance, "arcana:hungry_node")) {
                playerWorld.addParticle(ArcanaParticles.ASPECT_GREED_PARTICLE, true, particleLocation.getX(), particleLocation.getY() + 1, particleLocation.getZ(), 0, 0, 0);
            }
        }
    }
}
