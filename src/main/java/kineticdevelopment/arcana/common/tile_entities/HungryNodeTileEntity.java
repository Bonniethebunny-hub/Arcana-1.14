...
public class HungryNodeTileEntity extends TileEntity implements ITickableTileEntity {
    ...
    public void tick() {
        if(world.isRemote()) {
            ...
            if(priority > 0) {
                playerWorld.addParticle(ArcanaParticles.HUNGRY_NODE_PARTICLE, true, getParticleX(), getParticleY(), getParticleZ(), 0, 0, 0);
            }
            if(priority > 1 && isBlockOnCursor(player, playerWorld)) {
                playerWorld.addParticle(ArcanaParticles.ASPECT_GREED_PARTICLE, true, getParticleX(), getParticleY() + 1, getParticleZ(), 0, 0, 0);
            }
        }
    }
    ...
    private boolean isBlockOnCursor(ClientPlayerEntity player, World playerWorld) {
        ...
        return playerWorld.getBlockState(new BlockPos(result.getHitVec())).getBlock().getRegistryName().toString().equals("arcana:hungry_node");
    }
}
