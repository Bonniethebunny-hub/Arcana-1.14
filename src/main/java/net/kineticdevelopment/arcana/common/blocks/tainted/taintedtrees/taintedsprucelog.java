...
public class taintedsprucelog extends RotatedPillarBlock {
    public taintedsprucelog(Properties properties) {
        super(properties);
        setRegistryName("taintedsprucelog");
    }

    @Override
    public boolean onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (player.getHeldItemMainhand().getItem() instanceof AxeItem) {
            worldIn.setBlockState(pos, ModBlocks.STRIPPEDTAINTEDBIRCHLOG.getDefaultState().with(RotatedPillarBlock.AXIS, state.get(RotatedPillarBlock.AXIS)), 11);
